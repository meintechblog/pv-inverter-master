...
def test_mqtt_connect_connack_accepted():
    ...
    from pv_inverter_proxy.venus_reader import _mqtt_connect

    mock_sock = MagicMock()
    mock_sock.recv.return_value = b"\x20\x02\x00\x00"

    with patch("pv_inverter_proxy.venus_reader.socket.socket", return_value=mock_sock):
...
def test_no_hardcoded_ips():
    ...
    import pv_inverter_proxy.venus_reader as vr
...
async def test_discover_portal_id_success():
    ...
    import struct
    from pv_inverter_proxy.venus_reader import discover_portal_id
    ...
    with patch("pv_inverter_proxy.venus_reader._mqtt_connect", return_value=mock_sock):
...
async def test_discover_portal_id_connection_error():
    ...
    with patch("pv_inverter_proxy.venus_reader._mqtt_connect", side_effect=ConnectionError("refused")):
        result = await discover_portal_id("10.0.0.1", 1883, timeout=1.0)
